...
{
    board *b1 = board_new(5, 5, MATRIX);
    board *b2 = board_new(3, 5, MATRIX);
    printf("*** testing board_new\n");
    printf("- expecting 5 5 0 0 : %d %d %d %d\n", b1->width, b1->height, 
    b1->type, b1->u.matrix[0][0]);
    printf("- expecting 3 5 0 0 : %d %d %d %d\n", b2->width, b2->height, 
    b2->type, b2->u.matrix[0][0]);
    board *b3 = board_new(1, 3, BITS);
    printf("- expecting error above:'ERROR: Bits implementation requested'\n");
    board *b4 = board_new(2, 0, MATRIX);
...
void evidence_board_show()
{
    ...
    board_show(board_new(5, 5, MATRIX));
    board_show(board_new(3, 5, MATRIX));
...
void evidence_board_get()
{
    ...
    cell a = board_get(board_new(5, 5, MATRIX), make_pos(4, 4));
    cell b = board_get(board_new(3, 5, MATRIX), make_pos(0, 2));
